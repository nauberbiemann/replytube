    ...
    <div className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-1 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold tracking-tight text-foreground">Contexto do canal</h2>
        </div>
        {context && !loading && (
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="flex h-9 w-full rounded-lg border border-input bg-background px-3 py-1.5 text-xs text-foreground shadow-xs focus:outline-none focus:ring-1 focus:ring-red-500 cursor-pointer"
          >
          ...
          <button
            ...
            className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-border bg-muted/50 hover:bg-muted px-2.5 py-1.5 text-xs font-medium text-foreground transition shadow-xs"
          >
      ...
      {context && !loading && (
        <div className="space-y-2 rounded-lg border border-border bg-muted/30 p-3.5 text-xs leading-relaxed">
          <div>
          ...
          {context.temasChave && context.temasChave.length > 0 && (
            <div className="flex flex-wrap gap-1 pt-1">
              {context.temasChave.map((tema) => (
                <span
                  key={tema}
                  className="rounded-md bg-background px-2 py-0.5 text-[11px] font-medium text-foreground border border-border shadow-2xs"
                >
                  {tema}
                </span>
              ))}
          ...
          {context.resumo && (
            <p className="pt-1 text-[11px] text-muted-foreground border-t border-border">
              {context.resumo}
            </p>
          )}
        </div>
      ...
      {/* Modal Dialog para Cadastro/Edição com Fundo Sólido e Opaque */}
      {modalOpen && (
        <div
            ...
            <form onSubmit={handleSave} className="space-y-3 pt-1">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-input bg-background px-3 py-2 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-red-500 shadow-2xs"
                  required
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-border">
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="rounded-lg border border-border bg-background px-3.5 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition"
                >
                  Cancelar
                </button>
                <button
